...
def start_command(message):
    ...
    name = clean_md(message.from_user.first_name)
...
def handle_callbacks(call):
    ...
    if call.data == "get_number":
        ...
        bot.edit_message_text("🌍 Select a Country (Auto-updated from Panel):", call.message.chat.id, call.message.message_id, parse_mode="Markdown", reply_markup=markup)
        ...
        user = get_user(user_id)
        name = clean_md(call.from_user.first_name)
        markup = telebot.types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            telebot.types.InlineKeyboardButton("📞 Get Number", callback_data="get_number"),
            telebot.types.InlineKeyboardButton("💼 Balance & Ref", callback_data="balance_menu")
            ...
            for cdr in cdrs:
                ...
                if number and otp_code:
                    msg = (
                        f"🔔 New OTP Received!\n\n"
                        f"📱 Number: {number}\n"
                        f"🏷 Service: {clean_md(service)}\n"
                        f"🔑 OTP Code: {otp_code}"
                    )
                    try:
                        bot.send_message(OTP_CHANNEL, msg, parse_mode="Markdown")
                    except Exception as e:
