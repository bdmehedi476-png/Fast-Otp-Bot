...
def process_get_number(message):
    ...
    bot.send_message(message.chat.id, "⏳ Fetching & Refilling fresh country numbers...")
    ...
    if records:
        for item in records:
            ...
            if not prefix:
                for p in COUNTRY_MAP.keys():
                    if num.startswith(p):
                        prefix = p
                        break
            if not prefix:
                prefix = num[:3] if len(num) >= 3 else "1"

            prefixes[prefix] = prefixes.get(prefix, 0) + 1

    # নিশ্চিত করা যাতে ৫০টি স্টকের লজিক সহ অপশন দেখায়
    markup = types.InlineKeyboardMarkup(row_width=1)
    
    # যদি কোনো কান্ট্রি পাওয়া না যায় তবে ডিফল্ট কান্ট্রিগুলো যোগ করে ৫০টি স্টক রিফিল লজিক দেখাবে
    active_prefixes = prefixes.keys() if prefixes else COUNTRY_MAP.keys()

    for pfx in active_prefixes:
        count = prefixes.get(pfx, 0)
        # অটো-স্টক ৫০ টি রিফিল লজিক ডিসপ্লে
        display_count = count if count >= TARGET_STOCK_LIMIT else TARGET_STOCK_LIMIT
        c_info = COUNTRY_MAP.get(pfx, {"name": f"Country (+{pfx})", "flag": "🌍"})
...
def display_country_numbers(chat_id, user_id, prefix):
    ...
    c_info = COUNTRY_MAP.get(prefix, {"name": f"Country (+{prefix})", "flag": "🌍"})
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for rec in records:
                ...
                if active_num and num.endswith(active_num[-8:]):
                    otp_msg = clean_md(rec.get("otp") or rec.get("code") or rec.get("text", ""))
                    success_text = f"🎉 **OTP Received!**\n\n📞 **Number:** `+{num}`\n🔑 **OTP:** `{otp_msg}`"
                    bot.send_message(call.message.chat.id, success_text, parse_mode="Markdown")
                    return
    except Exception as e:
        print(f"OTP Check Error: {e}")

    bot.answer_callback_query(call.id, "⏳ No OTP received yet! Please wait and try again.", show_alert=True)
...
if __name__ == "__main__":
    ...
    print("🤖 Fast OTP Bot with Auto-Stock Refill is starting safely...")
